 ...
 * 
 * <p>
 * This class is designed to be used as a singleton. The constructor requires a MetadataUtil instance. Each MetadataUtil instance is
 * typically associated with a single persistence unit (i.e. EntityManagerFactory). A JPASearchProcessor can only be used with
 * EntityManagers that are associated with the same persistence unit as the MetadataUtil. If an application has multiple persistence units,
 * it will need to have multiple corresponding Search Processors.
 * 
 * @author dwolverton
 */
public class JPASearchProcessor extends BaseSearchProcessor {
    ...
    /**
     * Search for objects based on the search parameters in the specified <code>ISearch</code> object. Uses the specified searchClass,
     * ignoring the searchClass specified on the search itself.
     * 
     * @see ISearch
     */
    ...
    public List search(EntityManager entityManager, Class<?> searchClass, ISearch search) {
        ...
        return transformResults(query.getResultList(), search, search.getResultMode(), search.getResultMapClass());
    }

    /**
     * Returns the total number of results that would be returned using the given <code>ISearch</code> if there were no paging or maxResult
     * limits.
     * 
     * @see ISearch
     */
    ...
    /**
     * Returns the total number of results that would be returned using the given <code>ISearch</code> if there were no paging or maxResult
     * limits. Uses the specified searchClass, ignoring the searchClass specified on the search itself.
     * 
     * @see ISearch
     */
    ...
    /**
     * Returns a <code>SearchResult</code> object that includes the list of results like <code>search()</code> and the total length like
    ...
    /**
     * Returns a <code>SearchResult</code> object that includes the list of results like <code>search()</code> and the total length like
     * <code>searchLength</code>. Uses the specified searchClass, ignoring the searchClass specified on the search itself.
     * 
     * @see ISearch
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Search for a single result using the given parameters. Uses the specified searchClass, ignoring the searchClass specified on the
     * search itself.
     */
    ...
    private Object transformResult(Object result, ISearch search) {
        ...
        results.add(result);
        return transformResults(results, search, search.getResultMode(), search.getResultMapClass()).get(0);
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    private List transformResults(List results, ISearch search, int resultMode, Class<?> resultClass) {
        if (results.size() == 0)
            return results;

        // if resultMapClass is specified, first map to RESULT_MAP then replace each map with a filled resultMapClass instance.
        if (resultClass != null) {
            try {
                List<Map<String, Object>> l = transformResults(results, search, ISearch.RESULT_MAP, null);

                List<Object> result = new ArrayList<Object>();
                for (Map<String, Object> m : l) {
                    Object instance = resultClass.newInstance();
                    for (String key : m.keySet()) {
                        JPAAnnotationMetadataUtil.setNestedProperties(instance, key, m.get(key));
                    }
                    result.add(instance);
                }
